...
use shadowsocks::{
    config::{
        ManagerAddr, Mode, ReplayAttackPolicy, ServerAddr, ServerConfig, ServerSource, ServerProtocol, ServerUser, ServerUserManager,
...
pub struct ServerInstanceConfig {
    ...
    /// Server's outboundon fwmark / address / interface to support split tunnel
    #[cfg(any(target_os = "linux", target_os = "android"))]
...
impl Config {
    ...
    pub fn check_integrity(&self) -> Result<(), Error> {
        ...
        for inst in &self.server {
            ...
            if let ServerProtocol::SS(ref server) = server.protocol() {
