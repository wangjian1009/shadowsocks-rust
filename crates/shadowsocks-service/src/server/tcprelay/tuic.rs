...
impl TcpServer {
    pub async fn create_tuic_server(
        context: Arc<ServiceContext>,
        svr_cfg: &ServerConfig,
        tuic_cfg: &TuicConfig,
        accept_opts: &AcceptOpts,
    ) -> io::Result<tuic::server::Server> {
        ...
        let socket = match svr_cfg.addr() {
            ServerAddr::SocketAddr(sa) => create_inbound_udp_socket(sa, accept_opts.ipv6_only).await?,
            ServerAddr::DomainName(domain, port) => {
                lookup_then!(&context.context(), domain, *port, |addr| {
                    create_inbound_udp_socket(&addr, accept_opts.ipv6_only).await
                })?
                .1
            }
        };
        ...
        let server = tuic::server::Server::init(
            ...
            idle_timeout,
            Arc::new(Box::new(ServerPolicy::new(context.clone(), svr_cfg.timeout()))),
