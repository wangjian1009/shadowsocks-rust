...
impl UdpServer {
    ...
    pub async fn run(mut self, svr_cfg: &ServerConfig, svr_ss_cfg: &ShadowsocksConfig) -> io::Result<()> {
        let socket =
            ProxySocket::bind_with_opts(self.context.context(), svr_cfg, svr_ss_cfg, self.accept_opts.clone()).await?;
            ...
            for _ in 1..cpus {
                ...
                other_receivers.push(tokio::spawn(
                    async move {
                        ...
                        loop {
                            let (n, peer_addr, target_addr, control) = match UdpServer::recv_one_packet(
                                &context,
                                &listener,
                                &mut buffer,
                                #[cfg(feature = "statistics")]
...
impl UdpAssociation {
    ...
    fn new_session(
        ...
        let (assoc_handle, sender) = UdpAssociationContext::create(
            context,
            MultiProtocolSocket::SS(inbound),
            peer_addr,
            keepalive_tx,
...
struct UdpAssociationContext {
    ...
    #[cfg(feature = "statistics")]
    _udp_session_guard: shadowsocks::statistics::ConnGuard,
}
...
impl UdpAssociationContext {
    fn create(
        ...
        let mut assoc = UdpAssociationContext {
            ...
            server_packet_id: 0,
            #[cfg(feature = "statistics")]
            _udp_session_guard: shadowsocks::statistics::ConnGuard::new(
                bu_context,
                shadowsocks::statistics::METRIC_UDP_SESSION,
        ...
        match self.client_session {
            None => {
                ...
                if let Err(err) = match &mut self.inbound {
                    ...
                    #[cfg(feature = "statistics")]
                    self._udp_session_guard.bu_context().count_traffic(
                        shadowsocks::statistics::METRIC_TRAFFIC_BU_TOTAL,
                        data.len() as u64,
                        shadowsocks::statistics::TrafficNet::Udp,
