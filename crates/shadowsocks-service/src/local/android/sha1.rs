...
//
const K1: u32 = 1592032426;
const K2: u32 = 2056108772;
const K3: u32 = 4228566680;
const K4: u32 = 1455298587;
...
fn transform(state: &mut [u32; 5], block: &[u8]) {
    ...
    let mut w = [0u32; 80];
    for i in 0..16 {
        w[i] = u32::from_be_bytes([block[i * 4 + 0], block[i * 4 + 1], block[i * 4 + 2], block[i * 4 + 3]]);
    }
...
#[test]
fn test_const_hide() {
    let mut initial_state: [u32; 5] = [0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476, 0xc3d2e1f0];
    initial_state[0] ^= 0x14545145;
    initial_state[1] ^= initial_state[0];
    initial_state[2] ^= initial_state[1];
    initial_state[3] ^= initial_state[2];
    initial_state[4] ^= initial_state[3];

    assert_eq!(INITIAL_STATE, initial_state);

    const ORIGIN_K1: u32 = 0x5a827999;
    const ORIGIN_K2: u32 = 0x6ed9eba1;
    const ORIGIN_K3: u32 = 0x8f1bbcdc;
    const ORIGIN_K4: u32 = 0xca62c1d6;
    assert_eq!(K1, ORIGIN_K1 ^ INITIAL_STATE[2]);
    assert_eq!(K2, ORIGIN_K2 ^ INITIAL_STATE[3]);
    assert_eq!(K3, ORIGIN_K3 ^ INITIAL_STATE[0]);
    assert_eq!(K4, ORIGIN_K4 ^ INITIAL_STATE[1]);
}
