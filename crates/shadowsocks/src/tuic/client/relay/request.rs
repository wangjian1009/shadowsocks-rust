...
impl Request {
    ...
    pub fn new_associate(channel_size: usize) -> (Self, AssociateSendPacketSender, AssociateRecvPacketReceiver) {
        let assoc_id = get_random_u32();
        let (pkt_send_tx, pkt_send_rx) = mpsc::channel(channel_size);
        let (pkt_recv_tx, pkt_recv_rx) = mpsc::channel(channel_size);
