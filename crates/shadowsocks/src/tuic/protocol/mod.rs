...
pub enum Command {
    ...
    Packet { assoc_id: u32, len: u16, addr: Address },
    Dissociate { assoc_id: u32 },
    Heartbeat,
}
...
impl Command {
        ...
        R: AsyncRead + Unpin,
    {
        ...
        match cmd {
            ...
            _ => Err(Error::new(ErrorKind::InvalidInput, format!("Invalid command: {cmd}"))),
        }
    }
        ...
        match addr_type {
            Self::TYPE_DOMAIN => {
                ...
                let addr = String::from_utf8(buf)
                    .map_err(|err| Error::new(ErrorKind::InvalidData, format!("Invalid address encoding: {err}")))?;
