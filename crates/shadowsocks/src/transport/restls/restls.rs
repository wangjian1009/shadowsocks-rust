...
use super::{
    ...
    utils::{xor_bytes, TLSStream},
};

pub struct TryHandshake {}

impl TryHandshake {
    async fn read_from_stream<S>(&self, stream: &mut TLSStream<S>) -> Result<()>
    where
    ...
    async fn try_read_client_hello<S>(&self, inbound: &mut TLSStream<S>) -> Result<ClientHello>
    where
        S: AsyncRead + AsyncWrite + Unpin,
    {
    ...
    async fn try_read_server_hello<S>(&mut self, outbound: &mut TLSStream<S>) -> Result<ServerHello>
    where
        S: AsyncRead + AsyncWrite + Unpin,
    {
    ...
    async fn try_read_tls13_till_first_0x17<SI, SO>(
        &mut self,
        server_hello: &ServerHello,
        restls_password: &[u8],
        outbound: &mut TLSStream<SO>,
        inbound: &mut TLSStream<SI>,
    ) -> Result<()>
    where
        SI: AsyncRead + AsyncWrite + Unpin,
        SO: AsyncRead + AsyncWrite + Unpin,
    {
    ...
    async fn try_read_till_client_application_data<SI, SO>(
        &mut self,
    ...
    fn check_tls_13_application_data_auth(
        ...
        if application_data.len() < REQUIRED_HMAC_LEN {
            return Err(anyhow!("reject: application data too short to contain an auth header"));
        }
    ...
    async fn relay_to<SI, SO>(&mut self, to_stream: &mut TLSStream<SO>, from_stream: &mut TLSStream<SI>) -> Result<()>
    where
        SI: AsyncRead + AsyncWrite + Unpin,
        SO: AsyncRead + AsyncWrite + Unpin,
    {
        ...
        let res = match to_stream.get_mut().write_all(&from_stream.codec().raw_buf()).await {
            Ok(()) => Ok(()),
            Err(e) => Err(e.into()),
        };
    ...
    pub async fn try_handshake<SI, SO>(
        &mut self,
        options: &RestlsConfig,
        outbound: &mut TLSStream<SO>,
        inbound: &mut TLSStream<SI>,
    ) -> Result<usize>
    where
        ...
        SO: AsyncRead + AsyncWrite + Unpin,
    {
        ...
        if server_hello.is_tls_13 {
            ...
            self.relay_to(inbound, outbound).await?;
            self.try_read_till_client_application_data(2, outbound, inbound).await?;
            self.check_tls_13_application_data_auth(&server_hello, password, inbound.codec().peek_record())?;
...
// pub async fn handle(options: Arc<RestlsConfig>, inbound: TcpStream) -> Result<()> {
...
//     match try_handshake.try_handshake(&options, &mut outbound, &mut inbound).await {
//         Ok(discard) => {
...
//     );
//     loop {
//         let (stream, _) = listener.accept().await.context("failed to accept inbound stream")?;
