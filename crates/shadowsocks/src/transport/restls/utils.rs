...
use super::super::{Device, StreamConnection};

pub type TLSStream<S> = Framed<S, TLSCodec>;
...
impl RecordChecker {
    fn check(&mut self, record: &[u8]) -> bool {
        ...
        match self {
            ...
            InboundAfterClientHello => record[0] >= 0x14 && record[0] <= 0x18 && &record[1..3] == &[0x03, 0x03],
        }
    }
}
...
pub(crate) fn extend_from_length_prefixed<const N: usize, T: Buf>(buf: &mut T, copy_to: &mut Vec<u8>) {
    let len = read_length_padded_header::<N, T>(buf);
...
pub(crate) fn length_prefixed<const N: usize, T: Buf, P: FnOnce(Cursor<&[u8]>)>(buf: &mut T, parse: P) {
...
pub async fn copy_bidirectional_fallback<SI, SO>(mut inbound: TLSStream<SI>, mut outbound: TLSStream<SO>) -> Result<()>
where
    SI: StreamConnection,
    SO: StreamConnection,
{
    ...
    if inbound.codec().has_content() {
        ...
        debug!("write old msg to inbound {}", inbound.codec().raw_buf().len());
        outbound.get_mut().write_all(inbound.codec().raw_buf()).await?;
    }
    if outbound.codec().has_content() {
        outbound.codec_mut().skip_to_end();
        debug!("write old msg to outbound {}", outbound.codec().raw_buf().len());
                            ...
                            Some(e) => {
                                match e.kind() {
                                    io::ErrorKind::ConnectionReset => {
                                        inbound.get_mut().physical_device().apply(|f| {
                                            if let Device::Tcp(s) = f {
                                                s.set_linger(Some(Duration::from_secs(0)))
                                            }
                                            else {
                                                Ok(())
                                            }
