...
impl<S: StreamConnection> WebsocketStream<S> {
    ...
    fn step_check_content(
        ...
        match self.read_frame_opcode {
            ...
            _ => {
                tracing::warn!("Ignoring unknown frame type: {:?}", self.read_frame_opcode);
                if self.read_frame_length == 0 {
                    self.read_state = ReadState::Init;
                    self.step_init(cx, buf)
                } else {
...
impl<S: StreamConnection> AsyncRead for WebsocketStream<S> {
    fn poll_read(
        self: Pin<&mut Self>,
        cx: &mut Context<'_>,
        buf: &mut ReadBuf<'_>,
...
impl<S: StreamConnection> AsyncWrite for WebsocketStream<S> {
    fn poll_write(
        self: Pin<&mut Self>,
        cx: &mut Context<'_>,
        buf: &[u8],
...
impl<S:StreamConnection> StreamConnection for WebsocketStream<S> {
    ...
    fn set_rate_limit(&mut self, rate_limit: Option<Arc<RateLimiter>>) {
        self.stream.set_rate_limit(rate_limit)
    }
