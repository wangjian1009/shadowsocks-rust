...
fn get_ifindex(name: &[u8; libc::IFNAMSIZ]) -> i32 {
    debug_assert_eq!(name[libc::IFNAMSIZ - 1], 0, "name buffer not null-terminated");
...
impl Status for LinuxTunStatus {
    ...
    fn event(&mut self) -> Result<TunEvent, Self::Error> {
        ...
        tracing::debug!("netlink, fetch event (fd = {})", self.fd);
        loop {
            ...
            // read message
            let size: libc::ssize_t = unsafe { libc::recv(self.fd, mem::transmute(&mut buf), buf.len(), 0) };
            if size < 0 {
            ...
            tracing::debug!("netlink, received message ({} bytes)", size);
                    ...
                    libc::RTM_NEWLINK => {
                        ...
                        if info.ifi_index == self.index {
                            ...
                            if info.ifi_flags & (libc::IFF_UP as u32) != 0 {
                                ...
                                tracing::trace!("netlink, up event, mtu = {}", mtu);
                                self.events.push(TunEvent::Up(mtu));
                            } else {
                                tracing::trace!("netlink, down event");
