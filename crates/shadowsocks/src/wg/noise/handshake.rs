...
fn aead_chacha20_open(
    ...
    aead_chacha20_open_inner(buffer, key, nonce, data, aad).map_err(|_| WireGuardError::InvalidAeadTag)?;
    Ok(())
}
...
impl TimeStamper {
    ...
    pub fn new() -> TimeStamper {
        TimeStamper {
            duration_at_start: SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap(),
...
impl Handshake {
    ...
    pub(super) fn receive_handshake_response(&mut self, packet: HandshakeResponse) -> Result<Session, WireGuardError> {
    ...
    pub(super) fn receive_cookie_reply(&mut self, packet: PacketCookieReply) -> Result<(), WireGuardError> {
        ...
        let cookie = plaintext.try_into().map_err(|_| WireGuardError::InvalidPacket)?;
    ...
    pub(super) fn format_handshake_initiation<'a>(
        ...
        // msg.unencrypted_ephemeral = DH_PUBKEY(initiator.ephemeral_private)
        unencrypted_ephemeral.copy_from_slice(x25519_dalek::PublicKey::from(&ephemeral_private).as_bytes());
        // initiator.hash = HASH(initiator.hash || msg.unencrypted_ephemeral)
    ...
    fn format_handshake_response<'a>(&mut self, dst: &'a mut [u8]) -> Result<(&'a mut [u8], Session), WireGuardError> {
