...
impl Tunn {
    ...
    /// packet is processed.
    pub fn decapsulate<'a>(&self, src_addr: Option<IpAddr>, datagram: &[u8], dst: &'a mut [u8]) -> TunnResult<'a> {
    ...
    pub fn format_handshake_initiation<'a>(&self, dst: &'a mut [u8], force_resend: bool) -> TunnResult<'a> {
        self.inner.write().format_handshake_initiation(dst, force_resend)
    }
...
impl TunnInner {
    fn new(
        ...
        let tunn = TunnInner {
            ...
            rate_limiter: rate_limiter
                .unwrap_or_else(|| Arc::new(RateLimiter::new(&static_public, PEER_HANDSHAKE_RATE_LIMIT))),
        };
    ...
    fn set_static_private(
        ...
        self.rate_limiter =
            rate_limiter.unwrap_or_else(|| Arc::new(RateLimiter::new(&static_public, PEER_HANDSHAKE_RATE_LIMIT)));
    ...
    fn decapsulate<'a>(&mut self, src_addr: Option<IpAddr>, datagram: &[u8], dst: &'a mut [u8]) -> TunnResult<'a> {
        ...
        let packet = match self.rate_limiter.verify_packet(src_addr, datagram, &mut cookie) {
            Ok(packet) => packet,
    ...
    pub(crate) fn handle_verified_packet<'a>(&mut self, packet: Packet, dst: &'a mut [u8]) -> TunnResult<'a> {
        match packet {
    ...
    fn handle_handshake_init<'a>(
        ...
        tracing::debug!(remote_idx = p.sender_idx, "Received handshake_initiation");
    ...
    fn handle_cookie_reply<'a>(&mut self, p: PacketCookieReply) -> Result<TunnResult<'a>, WireGuardError> {
        tracing::debug!(local_idx = p.receiver_idx, "Received cookie_reply");
    ...
    fn set_current_session(&mut self, new_idx: usize) {
        ...
        if self.sessions[cur_idx % N_SESSIONS].is_none()
            || self.timers.session_timers[new_idx % N_SESSIONS] >= self.timers.session_timers[cur_idx % N_SESSIONS]
        {
            self.current = new_idx;
            tracing::debug!(session = new_idx, "New session");
        }
    }

    /// Decrypts a data packet, and stores the decapsulated packet in dst.
    fn handle_data<'a>(&mut self, packet: PacketData, dst: &'a mut [u8]) -> Result<TunnResult<'a>, WireGuardError> {
        ...
        let decapsulated_packet = {
            ...
            let session = session.ok_or_else(|| {
                tracing::trace!(remote_idx = r_idx, "No current session available");
    ...
    fn format_handshake_initiation<'a>(&mut self, dst: &'a mut [u8], force_resend: bool) -> TunnResult<'a> {
    ...
    fn validate_decapsulated_packet<'a>(&mut self, packet: &'a mut [u8]) -> TunnResult<'a> {
        let (computed_len, src_ip_address) = match packet.len() {
            ...
            _ if packet[0] >> 4 == 4 && packet.len() >= IPV4_MIN_HEADER_SIZE => {
                let len_bytes: [u8; IP_LEN_SZ] = packet[IPV4_LEN_OFF..IPV4_LEN_OFF + IP_LEN_SZ].try_into().unwrap();
            ...
            _ if packet[0] >> 4 == 6 && packet.len() >= IPV6_MIN_HEADER_SIZE => {
                let len_bytes: [u8; IP_LEN_SZ] = packet[IPV6_LEN_OFF..IPV6_LEN_OFF + IP_LEN_SZ].try_into().unwrap();
