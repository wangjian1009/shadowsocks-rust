...
cfg_if! {
    if #[cfg(feature = "rate-limit")] {
        use nonzero_ext::*;
        use crate::transport::NegativeMultiDecision;
    }
}
...
// constants
use super::constants::{
    DURATION_UNDER_LOAD, MAX_QUEUED_INCOMING_HANDSHAKES, MESSAGE_PADDING_MULTIPLE, THRESHOLD_UNDER_LOAD,
};
...
pub fn tun_worker<T: Tun, B: UDP>(wg: &WireGuard<T, B>, reader: T::Reader) {
    loop {
        ...
        tracing::trace!("TUN worker, payload length = {}, padded length = {}", payload, padded);
...
pub fn handshake_worker<T: Tun, B: UDP>(wg: &WireGuard<T, B>, rx: Receiver<HandshakeJob<B::Endpoint>>) {
    ...
    for job in rx {
        ...
        if pending > THRESHOLD_UNDER_LOAD {
            tracing::trace!("{} : handshake worker, under load (above threshold)", wg);
        ...
        if !under_load {
            ...
            if DURATION_UNDER_LOAD >= elapsed {
                tracing::trace!("{} : handshake worker, under load (recent)", wg);
                under_load = true;
            }
        }
                ...
                match device.process(
                    ...
                    Ok((peer, resp, keypair)) => {
                        ...
                        if let Some(peer) = peer {
                            ...
                            peer.opaque().tx_bytes.fetch_add(resp_len, Ordering::Relaxed);
