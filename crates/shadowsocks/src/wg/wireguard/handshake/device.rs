...
impl<'a, O> Iterator for Iter<'a, O> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.iter.next().map(|(pk, peer)| (PublicKey::from(*pk), &peer.opaque))
    }
}
...
impl<O> Device<O> {
    ...
    pub fn begin<R: RngCore + CryptoRng>(&self, rng: &mut R, pk: &PublicKey) -> Result<Vec<u8>, HandshakeError> {
        match (self.keyst.as_ref(), self.pk_map.get(pk.as_bytes())) {
            ...
            (Some(keyst), Some(peer)) => {
                ...
                peer.macs.lock().generate(msg.noise.as_bytes(), &mut msg.macs);
            ...
            TYPE_INITIATION => {
                ...
                if let Some(src) = src {
                    ...
                    if !keyst.macs.check_mac2(msg.noise.as_bytes(), &src, &msg.macs) {
                        ...
                        keyst
                            .macs
                            .create_cookie_reply(rng, msg.noise.f_sender.get(), &src, &msg.macs, &mut reply);
                ...
                let keys = noise::create_response(rng, peer, &pk, local, st, &mut resp.noise).map_err(|e| {
                    self.release(local);
                    e
                })?;

                // add macs to response
                peer.macs.lock().generate(resp.noise.as_bytes(), &mut resp.macs);

                // return unconfirmed keypair and the response as vector
                Ok((Some(&peer.opaque), Some(resp.as_bytes().to_owned()), Some(keys)))
            }
            TYPE_RESPONSE => {
    ...
    pub(super) fn lookup_pk(&self, pk: &PublicKey) -> Result<&Peer<O>, HandshakeError> {
        self.pk_map.get(pk.as_bytes()).ok_or(HandshakeError::UnknownPublicKey)
    }
    ...
    pub(super) fn lookup_id(&self, id: u32) -> Result<(&Peer<O>, PublicKey), HandshakeError> {
        ...
        let pk = self.id_map.get(&id).ok_or(HandshakeError::UnknownReceiverId)?;
