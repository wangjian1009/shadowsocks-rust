...
pub struct SendJob<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>>(Arc<Inner<E, C, T, B>>);
...
impl<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>> SendJob<E, C, T, B> {
    pub fn new(buffer: Vec<u8>, counter: u64, keypair: Arc<KeyPair>, peer: Peer<E, C, T, B>) -> SendJob<E, C, T, B> {
...
impl<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>> ParallelJob for SendJob<E, C, T, B> {
    ...
    fn parallel_work(&self) {
        ...
        tracing::trace!("processing parallel send job");
        ...
        // encrypt body
        {
            ...
            let key = LessSafeKey::new(UnboundKey::new(&CHACHA20_POLY1305, &job.keypair.send.key[..]).unwrap());
            let tag = key
...
impl<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>> SequentialJob for SendJob<E, C, T, B> {
    ...
    async fn sequential_work(self) {
        ...
        tracing::trace!("processing sequential send job");
