    ...
    pub fn get_route(&self, packet: &[u8]) -> Option<T> {
        match packet.get(0)? >> 4 {
            VERSION_IP4 => {
                ...
                let (header, _): (LayoutVerified<&[u8], IPv4Header>, _) = LayoutVerified::new_from_prefix(packet)?;
            ...
            VERSION_IP6 => {
                ...
                let (header, _): (LayoutVerified<&[u8], IPv6Header>, _) = LayoutVerified::new_from_prefix(packet)?;
            ...
            v => {
                tracing::trace!("router, invalid IP version {}", v);
                None
            }
        }
    }
