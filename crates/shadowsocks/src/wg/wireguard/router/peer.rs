...
impl<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>> Deref for PeerHandle<E, C, T, B> {
    type Target = PeerInner<E, C, T, B>;
    fn deref(&self) -> &Self::Target {
        &self.peer
    }
...
impl<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>> fmt::Display for PeerHandle<E, C, T, B> {
...
impl<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>> Drop for PeerHandle<E, C, T, B> {
    fn drop(&mut self) {
        ...
        *peer.endpoint.lock() = None;
        ...
        tracing::debug!("peer dropped & removed from device");
    }
}
    ...
    pub(super) async fn send(&self, msg: Vec<u8>, stage: bool) {
        ...
        let (job, need_key) = {
            ...
            match enc_key.as_mut() {
                None => {
                    tracing::debug!("no key encryption key available");
                    if stage {
                ...
                Some(mut state) => {
                    ...
                    if state.nonce >= REJECT_AFTER_MESSAGES - 1 {
                        tracing::debug!("encryption key expired");
                        *enc_key = None;
                        if stage {
                            self.staged_packets.lock().push_back(msg);
                        }
                        ...
                        tracing::debug!("encryption state available, nonce = {}", state.nonce);
...
impl<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>> PeerHandle<E, C, T, B> {
    ...
    pub fn set_endpoint(&self, endpoint: E) {
        tracing::trace!("peer.set_endpoint");
    ...
    pub async fn add_keypair(&self, new: KeyPair) -> Vec<u32> {
        ...
        let release = {
            ...
            {
                tracing::trace!("peer.add_keypair: updating inbound id map");
                let mut recv = self.peer.device.recv.write();
                ...
                recv.insert(new.recv.id, Arc::new(DecryptionState::new(self.peer.clone(), &new)));
            }
            release
        };
        ...
        debug_assert!(release.len() <= 3, "since the key-wheel contains at most 3 keys");
        release
    }
    ...
    pub fn add_allowed_ip(&self, ip: IpAddr, masklen: u32) {
        self.peer.device.table.insert(ip, masklen, self.peer.clone())
    }
