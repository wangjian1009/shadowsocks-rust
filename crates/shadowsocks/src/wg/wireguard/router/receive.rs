...
pub struct ReceiveJob<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>>(Arc<Inner<E, C, T, B>>);

impl<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>> Clone for ReceiveJob<E, C, T, B> {
...
impl<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>> ReceiveJob<E, C, T, B> {
    pub fn new(buffer: Vec<u8>, state: Arc<DecryptionState<E, C, T, B>>, endpoint: E) -> ReceiveJob<E, C, T, B> {
...
impl<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>> ParallelJob for ReceiveJob<E, C, T, B> {
    ...
    fn parallel_work(&self) {
        debug_assert_eq!(self.is_ready(), false, "doing parallel work on completed job");
        tracing::trace!("processing parallel receive job");
        ...
        // decrypt
        {
            ...
            let ok = (|| {
                ...
                let key =
                    LessSafeKey::new(UnboundKey::new(&CHACHA20_POLY1305, &job.state.keypair.recv.key[..]).unwrap());
...
impl<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>> SequentialJob for ReceiveJob<E, C, T, B> {
    ...
    fn sequential_work(self) {
        ...
        if !job.state.protector.lock().update(header.f_counter.get()) {
            tracing::debug!("inbound worker: replay detected");
            return;
        }
