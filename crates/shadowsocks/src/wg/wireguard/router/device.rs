...
cfg_if! {
    if #[cfg(feature = "rate-limit")] {
        use nonzero_ext::*;
        use crate::transport::{NegativeMultiDecision, RateLimiter};
    }
}
...
impl<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>> PartialEq for Device<E, C, T, B> {
...
impl<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>> Drop for DeviceHandle<E, C, T, B> {
    fn drop(&mut self) {
        tracing::debug!("router: dropping device");
...
impl<E: Endpoint, C: Callbacks, T: tun::Writer, B: udp::Writer<E>> DeviceHandle<E, C, T, B> {
    ...
    pub fn send_raw(
        ...
        msg: &[u8],
        dst: &mut E,
        #[cfg(feature = "rate-limit")] rate_limit: &RateLimiter,
    ) -> Result<(), B::Error> {
    ...
    pub fn clear_sending_keys(&self) {
        tracing::debug!("Clear sending keys");
