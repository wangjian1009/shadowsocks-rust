...
use super::super::serialization::KeyBytes;
...
fn create_sock_dir() {
    ...
    if let Ok((saved_uid, saved_gid)) = get_saved_ids() {
        unsafe {
            ...
            chown(c_path.as_bytes_with_nul().as_ptr() as _, saved_uid, saved_gid);
        }
    }
}
                ...
                }
                {
                    ...
                    match key {
                        ...
                        "public_key" => match val.parse::<KeyBytes>() {
                            ...
                            Ok(key_bytes) => {
                                return api_set_peer(reader, device, x25519_dalek::PublicKey::from(key_bytes.0))
                            }
                            Err(_) => return EINVAL,
                        },
                        _ => return EINVAL,
...
fn api_set_peer(reader: &mut BufReader<&UnixStream>, d: &mut Device, pub_key: x25519_dalek::PublicKey) -> i32 {
    let mut cmd = String::new();
