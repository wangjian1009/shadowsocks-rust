...
mod tests {
    ...
    fn test_allowed_ips_insert_find() {
        ...
        assert_eq!(map.find(IpAddr::from([553, 0, 0, 1, 0, 0, 0, 0])), Some(&'7'));
    ...
    fn test_allowed_ips_remove() {
        ...
        assert_eq!(map_iter.next(), Some((&'6', IpAddr::from([45, 25, 15, 0]), 30)));
        assert_eq!(map_iter.next(), Some((&'2', IpAddr::from([127, 0, 0, 0]), 16)));
        assert_eq!(map_iter.next(), Some((&'3', IpAddr::from([127, 1, 15, 0]), 24)));
        assert_eq!(map_iter.next(), Some((&'4', IpAddr::from([255, 1, 15, 0]), 24)));
    ...
    fn test_allowed_ips_iter() {
        ...
        let mut map_iter = map.iter();
        assert_eq!(map_iter.next(), Some((&'6', IpAddr::from([45, 25, 15, 0]), 30)));
        assert_eq!(map_iter.next(), Some((&'5', IpAddr::from([60, 25, 15, 1]), 32)));
        assert_eq!(map_iter.next(), Some((&'2', IpAddr::from([127, 0, 0, 0]), 16)));
        assert_eq!(map_iter.next(), Some((&'1', IpAddr::from([127, 0, 0, 1]), 32)));
        assert_eq!(map_iter.next(), Some((&'3', IpAddr::from([127, 1, 15, 0]), 24)));
        assert_eq!(map_iter.next(), Some((&'4', IpAddr::from([255, 1, 15, 0]), 24)));
        assert_eq!(
            map_iter.next(),
    ...
    fn test_allowed_ips_v6_kernel_compatibility() {
        ...
        map.insert(
            IpAddr::from([0x2607, 0x5300, 0x6000, 0x6b00, 0x0000, 0x0000, 0xc05f, 0x0543]),
            128,
            'd',
        );
        map.insert(
            IpAddr::from([0x2607, 0x5300, 0x6000, 0x6b00, 0x0000, 0x0000, 0x0000, 0x0000]),
            64,
            'c',
        );
        map.insert(
            IpAddr::from([0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000]),
            0,
            'e',
        );
        map.insert(
        ...
        map.insert(
            IpAddr::from([0x2404, 0x6800, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000]),
            32,
            'g',
        );
        map.insert(
            IpAddr::from([0x2404, 0x6800, 0x4004, 0x0800, 0xdead, 0xbeef, 0xdead, 0xbeef]),
            64,
            'h',
        );
        map.insert(
        ...
        map.insert(
            IpAddr::from([0x2444, 0x6800, 0x40e4, 0x0800, 0xdeae, 0xbeef, 0x0def, 0xbeef]),
            128,
            'c',
        );
        map.insert(
            IpAddr::from([0x2444, 0x6800, 0xf0e4, 0x0800, 0xeeae, 0xbeef, 0x0000, 0x0000]),
            98,
            'b',
        );
        ...
        assert_eq!(
            Some(&'c'),
            map.find(IpAddr::from([0x2607, 0x5300, 0x6000, 0x6b00, 0, 0, 0xc02e, 0x01ee]))
        );
        assert_eq!(
            Some(&'f'),
        ...
        assert_eq!(
            Some(&'g'),
            map.find(IpAddr::from([0x2404, 0x6800, 0x4004, 0x0806, 0, 0, 0, 0x1006]))
        );
        assert_eq!(
            Some(&'g'),
            map.find(IpAddr::from([0x2404, 0x6800, 0x4004, 0x0806, 0, 0x1234, 0, 0x5678]))
        );
        assert_eq!(
            Some(&'f'),
            map.find(IpAddr::from([0x2404, 0x67ff, 0x4004, 0x0806, 0, 0x1234, 0, 0x5678]))
        );
        assert_eq!(
            Some(&'f'),
            map.find(IpAddr::from([0x2404, 0x6801, 0x4004, 0x0806, 0, 0x1234, 0, 0x5678]))
        );
        assert_eq!(
            Some(&'h'),
            map.find(IpAddr::from([0x2404, 0x6800, 0x4004, 0x0800, 0, 0x1234, 0, 0x5678]))
        );
        assert_eq!(
            Some(&'h'),
    ...
    fn test_allowed_ips_iter_zero_leaf_bits() {
        ...
        let mut map_iter = map.iter();
        assert_eq!(map_iter.next(), Some((&'1', IpAddr::from([10, 111, 0, 1]), 32)));
        assert_eq!(map_iter.next(), Some((&'2', IpAddr::from([10, 111, 0, 2]), 32)));
        assert_eq!(map_iter.next(), Some((&'3', IpAddr::from([10, 111, 0, 3]), 32)));
        assert_eq!(map_iter.next(), None);
    }
}
