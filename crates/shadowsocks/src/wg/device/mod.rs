...
use self::tun::{errno, errno_str, TunSocket};
use super::noise::errors::WireGuardError;
use super::noise::handshake::parse_handshake_anon;
use super::noise::rate_limiter::RateLimiter;
    ...
    fn event_loop(_i: usize, device: &Lock<Device>) {
        ...
        let mut thread_local = ThreadData {
            ...
            iface: if _i == 0 || !device.read().config.use_multi_queue {
                ...
                device.read().register_iface_handler(Arc::clone(&iface_local)).ok();
...
impl Device {
    ...
    fn remove_peer(&mut self, pub_key: &x25519_dalek::PublicKey) {
        if let Some(peer) = self.peers.remove(pub_key) {
            ...
            self.peers_by_ip.remove(&|p: &Arc<Peer>| Arc::ptr_eq(&peer, p)); // peers_by_ip
    ...
    fn update_peer(
        ...
        let next_index = self.next_index();
        let device_key_pair = self.key_pair.as_ref().expect("Private key must be set first");
    ...
    fn open_listen_socket(&mut self, mut port: u16) -> Result<(), Error> {
        ...
        let udp_sock4 = Arc::new(UDPSocket::new()?.set_non_blocking()?.set_reuse()?.bind(port)?);
        ...
        let udp_sock6 = Arc::new(UDPSocket::new6()?.set_non_blocking()?.set_reuse()?.bind(port)?);
    ...
    fn set_key(&mut self, private_key: x25519_dalek::StaticSecret) {
        ...
        for peer in self.peers.values_mut() {
            let peer_mut = Arc::<Peer>::get_mut(peer).expect("set_key requires other threads to be stopped");

            if peer_mut
                .tunnel
                .set_static_private(private_key.clone(), public_key, Some(Arc::clone(&rate_limiter)))
                .is_err()
            {
    ...
    pub(crate) fn trigger_yield(&self) {
        self.queue.trigger_notification(self.yield_notice.as_ref().unwrap())
    }

    pub(crate) fn trigger_exit(&self) {
        self.queue.trigger_notification(self.exit_notice.as_ref().unwrap())
    }

    pub(crate) fn cancel_yield(&self) {
        self.queue.stop_notification(self.yield_notice.as_ref().unwrap())
    }
        ...
        self.queue.new_event(
            ...
            Box::new(move |d, t| {
                ...
                while let Ok((addr, packet)) = udp.recvfrom(&mut t.src_buf[..]) {
                    ...
                    let parsed_packet = match rate_limiter.verify_packet(Some(addr.ip()), packet, &mut t.dst_buf) {
                        Ok(packet) => packet,
                    ...
                    let peer = match &parsed_packet {
                        Packet::HandshakeInit(p) => parse_handshake_anon(private_key, public_key, p)
                            .ok()
                            .and_then(|hh| d.peers.get(&x25519_dalek::PublicKey::from(hh.peer_static_public))),
                    ...
                    match peer.tunnel.handle_verified_packet(parsed_packet, &mut t.dst_buf[..]) {
                        TunnResult::Done => {}
    ...
    fn register_conn_handler(&self, peer: Arc<Peer>, udp: Arc<UDPSocket>, peer_addr: IpAddr) -> Result<(), Error> {
        self.queue.new_event(
            udp.as_raw_fd(),
            Box::new(move |_, t| {
