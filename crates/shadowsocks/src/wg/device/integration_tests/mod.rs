...
mod tests {
    use crate::wg::device::{DeviceConfig, DeviceHandle};
    ...
    fn next_ip() -> IpAddr {
        IpAddr::V4(Ipv4Addr::from(NEXT_IP.fetch_add(1, Ordering::Relaxed) as u32))
    }
    ...
    impl Peer {
        ...
        fn gen_wg_conf(&self, local_key: &PublicKey, local_addr: &IpAddr, local_port: u16) -> String {
            ...
            let _ = writeln!(conf, "PrivateKey = {}", base64encode(&self.key.clone().to_bytes()));
        ...
        fn get_request(&self) -> String {
            ...
            tcp_conn.set_read_timeout(Some(std::time::Duration::from_secs(60))).ok();
    ...
    impl WGHandle {
        ...
        fn start(&mut self) {
            Command::new("ip")
                .args(&["address", "add", &self.addr_v4.to_string(), "dev", &self.name])
                .status()
                .expect("failed to assign ip to tunnel");

            Command::new("ip")
                .args(&["address", "add", &self.addr_v6.to_string(), "dev", &self.name])
                .status()
                .expect("failed to assign ipv6 to tunnel");
            ...
            for p in &self.peers {
                for r in &p.allowed_ips {
                    Command::new("ip")
                        .args(&["route", "add", &format!("{}/{}", r.ip, r.cidr), "dev", &self.name])
                        .status()
                        .expect("failed to add route");
                }
            }
        ...
        fn add_peer(&mut self, peer: Arc<Peer>) {
            self.wg_set_peer(&PublicKey::from(&peer.key), &peer.endpoint, &peer.allowed_ips);
    ...
    fn test_wireguard_set() {
        ...
        assert_eq!(wg.wg_set_peer(&peer_pub_key, &endpoint, &allowed_ips), "errno=0\n\n");
    ...
    fn test_wg_start_ipv6_endpoint() {
        ...
        let mut peer = Peer::new(
            SocketAddr::new(IpAddr::V6(Ipv6Addr::new(0, 0, 0, 0, 0, 0, 0, 1)), next_port()),
            vec![AllowedIp {
                ip: next_ip_v6(),
                cidr: 128,
            }],
